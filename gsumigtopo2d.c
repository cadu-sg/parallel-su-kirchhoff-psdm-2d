...
char* sdoc[] = {
    " 									",
    " SUMIGTOPO2D - Kirchhoff Depth Migration of 2D postack/prestack "
    "data	",
    ...
    "	",
    "    other one that considers topography )on relatively coarse "
    "grids,	",
    "    with dimension ns*nxt*nzt. In the migration process, "
    "traveltimes	",
    ...
    "	",
    " 6. Input traces must specify source and receiver positions via the "
    "header",
...
void mig2d(float* trace, int nt, float ft, float dt, float sx, float gx,
           ...
           float angmax, float** tbs, float** tbg, float** pbs, float** pbg,
           float** angbs, float** angbg, int nr, float** tsum, int nzt,
           float fzt, float dzt, int nxt, float fxt, float dxt, float* szif,
...
int main(int argc, char** argv) {
  ...
  float v0, v00, dvz, fmax, angmax, offmax, rmax, aperx, sx, gx, nangls, nanglg;
  float ***mig, ***ttab, ***tb, ***pb, ***angb, **tsum, **tt, *szif, *nangl,
      *sz, **tbs, **tbg, **pbs, **pbg, **angbs, **angbg, ***sigb, **sigbs,
      **sigbg;
  Surface* srf;
  ...
  do {
    ...
    if (MIN(sx, gx) >= fs && MAX(sx, gx) <= es &&
      ...
      mig2d(tr.data, nt, ft, dt, sx, gx, mig[io], aperx, nxo, fxo, dxo, nzo,
            fzo, dzo, ls, mtmax, dxm, fmax, angmax, tbs, tbg, pbs, pbg, angbs,
            angbg, nr, tsum, nzt, fzt, dzt, nxt, fxt, dxt, szif, nangls, nanglg,
            sigbs, sigbg);
...
**************************************************************************/
{
  ...
  for (t = s, nxz = 0; t != NULL; ++nxz) t = strtok(NULL, ",;\0");
...
void makesurf(float dsmax, int nr, int* nu, float** xu, float** zu, Surface** r)
/*****************************************************************************
...
*****************************************************************************/
{
  ...
  float x, z, xlast, zlast, dx, dz, duu, uu, ds, fs, rsx, rsz, rsxd, rszd, *u,
      *s, (*xud)[4], (*zud)[4], *us;
  SurfaceSegment* ss;
  Surface* rr;
    ...
    for (ir = 0, r = 0; ir < nr; ++ir, r += dr)
      for (iz = 0, z = fz + zc - z0; iz < nz; ++iz, z += dz) {
        ...
        if (ir == 0) {
          ...
          p[ir][iz] = 0.0;
          ang[ir][iz] = 0.0;
          sig[ir][iz] = 0.5 * (z + 0.1 * dz - zc + z0) * (v0 + v);
        } else {
...
*****************************************************************************/
{
  ...
  float xm, x, dis, rxz, ar, srs, srg, srs0, srg0, sigp, z0, rdz, ampd, res0,
      angs, angg, ax, ax0, pmin, odt = 1.0 / dt, pd, az, sz, sz0, sigmas,
                                 sigmag, at, td, res, temp, sigs, sigg;
  ...
  for (ix = nxtf; ix <= nxte; ++ix) {
    ...
    for (iz = izt0; iz < nzt; ++iz) {
      ...
      pd = srs0 * pbs[jrs][iz] + srs * pbs[jrs + 1][iz] +
           sigp * (srg0 * pbg[jrg][iz] + srg * pbg[jrg + 1][iz]);
      if (pd < 0.0) pd = -pd;
      temp = pd * dxm * odt;
      if (temp < 1) temp = 1.0;
