...
char* sdoc[] = {
    " 									",
    "SUKDMIG2D - Kirchhoff Depth Migration of 2D poststack/prestack "
    "data	",
    " 									",
    ...
    "	",
    " nzt= 			number of depth samples in traveltime "
    "table	",
    " dzt=			depth interval in traveltime table	"
    "	",
    " fxt=			first lateral sample in traveltime "
    "table	",
    " nxt=			number of lateral samples in traveltime "
    "table	",
    ...
    " limoff=0      flag for only using input traces that fall within the "
    "range",
    ...
    " ntr=100000		maximum number of input traces to be "
    "migrated	",
    " npv=0			flag of computing quantities for velocity "
    "analysis",
    " rscale=1000.0 	scaling for roundoff error suppression		",
    "									",
    "   ...if npv>0 specify the following three files:			",
    " tvfile=tvfile		input file of traveltime variation "
    "tables	",
    "			tv[ns][nxt][nzt]				",
    " csfile=csfile		input file of cosine tables "
    "cs[ns][nxt][nzt]	",
    ...
    "	",
    " 7. Input traces must specify source and receiver positions via the "
    "header",
    "    fields tr.sx and tr.gx. Offset is computed automatically.	"
    "	",
    " 8. if limoff=0, input traces from outside the range defined by off0, "
    "doff, ",
    "    noff, will get migrated into the extremal offset bins/planes.  E.g. "
    "if ",
    "    absoff=0 and limoff=0, all traces with gx<sx will get migrated into "
    "the ",
    "    off0 bin.",
    "									",
    NULL};
...
void interpx(int nxt, float fxt, float dxt, int nx, float fx, float dx, int nzt,
             float** tt, float** t);
void sum2(int nx, int nz, float a1, float a2, float** t1, float** t2,
          float** t);
void timeb(int nr, int nz, float dr, float dz, float fz, float a, float v0,
           float** t, float** p, float** sig, float** ang);
void mig2d(float* trace, int nt, float ft, float dt, float sx, float gx,
           float** mig, float aperx, int nx, float fx, float dx, float nz,
           float fz, float dz, int ls, int mtmax, float dxm, float fmax,
           float angmax, float** tb, float** pb, float** cs0b, float** angb,
           int nr, float** tsum, int nzt, float fzt, float dzt, int nxt,
           float fxt, float dxt, int npv, float** cssum, float** tvsum,
           float** mig1);
...
int main(int argc, char** argv) {
  ...
  if (!getparstring("ttfile", &ttfile)) err("must specify ttfile!\n");
  ...
  if (fxt > fxo || ext < exo || fzt > fzo || ezt < ezo) {
    warn(
        "This condition must NOT be satisfied: fxt>fxo || ext<exo || fzt>fzo "
        "|| ezt<ezo");
    warn("fxt=%f fxo=%f ext=%f exo=%f fzt=%f fzo=%f ezt=%f ezo=%f", fxt, fxo,
         ext, exo, fzt, fzo, ezt, ezo);
  ...
  if (npv) {
    if (!getparstring("tvfile", &tvfile)) err("must specify tvfile!\n");
    tvfp = fopen(tvfile, "r");
    if (!getparstring("csfile", &csfile)) err("must specify csfile!\n");
    csfp = fopen(csfile, "r");
    if (!getparstring("outfile1", &outfile1)) outfile1 = "outfile1";
  ...
  if (npv) memset((void*)mig1[0][0], 0, noff * nxo * nzo * sizeof(float));
...
/* compute  reference traveltime and slowness	*/
void timeb(int nr, int nz, float dr, float dz, float fz, float a, float v0,
           float** t, float** p, float** cs0, float** ang) {
  int ir, iz;
    ...
    for (ir = 0, r = 0; ir < nr; ++ir, r += dr)
      for (iz = 0, z = fz + zc; iz < nz; ++iz, z += dz) {
        ...
        if (ir == 0) {
          ...
          t[ir][iz] = log((v * (rou + rc)) / (v0 * (rou + rc - r))) * oa;
          p[ir][iz] = sqrt(rou * rou - rc * rc) / (rou * v0);
          temp = v * p[ir][iz];
...
void filt(float* trace, int nt, float dt, float fmax, int ls, int m,
          float* trf);
...
*****************************************************************************/
{
  ...
  float xm, x, dis, rxz, ar, srs, srg, srs0, srg0, sigp, z0, rdz, ampd, res0,
      angs, angg, cs0s, cs0g, ax, ax0, pmin, odt = 1.0 / dt, pd, az, sz, sz0,
                                             at, td, res, temp;
  ...
  for (ix = nxtf; ix <= nxte; ++ix) {
    ...
    for (iz = izt0; iz < nzt; ++iz) {
      ...
      pd = srs0 * pb[jrs][iz] + srs * pb[jrs + 1][iz] +
           sigp * (srg0 * pb[jrg][iz] + srg * pb[jrg + 1][iz]);
      if (pd < 0.0) pd = -pd;
      temp = pd * dxm * odt;
      if (temp < 1) temp = 1.0;
  ...
  for (ix = nxf; ix <= nxe; ++ix) {
    ...
    for (iz = izt0; iz < nzt; ++iz) {
      ...
      if (npv) amp1[iz] = ax0 * ampt1[jx][iz] + ax * ampt1[jx + 1][iz];
    }
    ...
    for (iz = iz0; iz < nzp; ++iz) {
      ...
      if (jt > mtmax && jt < nt + mtmax - 1) {
        ...
        res0 = 1.0 - res;
        temp =
            (res0 * (-trf[jt - mt] + 2.0 * trf[jt] - trf[jt + mt]) +
             res * (-trf[jt - mt + 1] + 2.0 * trf[jt + 1] - trf[jt + mt + 1])) *
            ampd;
        mig[ix][iz] += temp;

        if (npv) mig1[ix][iz] += temp * (sz0 * amp1[jz] + sz * amp1[jz + 1]);
      }
    }
